...
local script_vers = 4 -- не менять
local script_vers_text = "1.4" -- не менять
...
text_auto = imgui.ImBuffer(tostring(config.auto.text), 256)
...
password = imgui.ImBuffer(tostring(config.settings.password), 256)
auto_password = imgui.ImBool(config.settings.auto_password)
--
...
function SecondChild()
	...
	imgui.SameLine()
	local icon
	if auto_send.v then icon = fa.ICON_FA_PAUSE_CIRCLE
	elseif not auto_send.v then icon = fa.ICON_FA_PLAY end
	if imgui.Button(icon .. '##1', imgui.ImVec2(20, 20)) then 
		...
		if auto_send.v then
			...
			msgChat("Задержка: ".. config.auto.wait .. " секунд")
		elseif not auto_send.v then
			msgChat("Авто-Текст завершен")
		end
		lua_thread.create(autosendfunc)
	end
	imgui.ToggleButton('Auto-Password', auto_password)
	if auto_password.v then imgui.InputText('##password', password) end
	imgui.SameLine()
	if imgui.Button(fa.ICON_FA_PASTE .. '##savepassword', imgui.ImVec2(25, 20)) then
		config.settings.password = password.v 
		config.settings.auto_password = auto_password.v 
		if inicfg.save(config,directIni) then
			if bNotf then
				notf.addNotification("Настройки авто-пароля сохранены", 3, 1)
			end
		end
	end
	imgui.EndChild()
...
function sampev.onShowDialog(id, style, title, button1, button2, text)
	if button1:match("Принимаю") and auto_password.v then
		lua_thread.create(function()
		sampSendDialogResponse(32700, 1, 1, nil)
		wait(1)
		sampSendDialogResponse(32700, 1, 0, password.v)
		end)
	end
end
function checkanim()
